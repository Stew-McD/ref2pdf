...
from config import FILE_IN, FILE_OUT_BIB, FILE_OUT_CSV
...
def bib2pdf_parallel(file_bib):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = list(tqdm(executor.map(process_row, df.itertuples()), total=total_tasks, desc="Processing Rows"))
